...
<head>
	...
	<title>Tailwind Starter Template - Profile Card : Tailwind Toolbox</title>
	<meta name="description" content="Free open source Tailwind CSS starter template - for use as a user profile website / card">
	<meta name="keywords" content="tailwind,tailwindcss,tailwind css,css,starter template,free template,profile, user profile, card">
	...
	<div class="max-w-4xl flex items-center h-auto lg:h-screen flex-wrap mx-auto my-32 lg:my-0">
		...
		<div id="profile" class="w-full lg:w-3/5 rounded-lg lg:rounded-l-lg lg:rounded-r-none shadow-2xl bg-white opacity-75 mx-6 lg:mx-0">
			...
			<div class="p-4 md:p-12 text-center lg:text-left">
				...
				<h1 class="text-3xl font-bold pt-8 lg:pt-0">{{ $link->profile->owner->email }}</h1>
				...
				@foreach ($link->profile->experiences()->orderBy('sort')->get() as $experience)
				<p class="pt-8 text-sm">
					{{ $experience->jobTitle->name }} à <strong>{{ $experience->company->name }}</strong> démarré le
					{{ $experience->started_at->format('d/m/Y') }} -
					{{ $experience->finished_at?->format('d/m/Y') }}
				</p>
				@endforeach
